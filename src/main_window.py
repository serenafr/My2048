...
class My2048_wx(wx.Frame):
	...
	def generate_header(self):
		...
		self.score = ST.GenStaticText(self, -1, label = '0', size = (50, 20), style = wx.ALIGN_CENTRE)
		...
		self.best = ST.GenStaticText(self, -1, label = '0', size = (50, 20), style = wx.ALIGN_CENTRE)
	...
	def generate_playboard(self):
		...
		for i in range(0, self.SIZE * self.SIZE):
			self.label_list.append(ST.GenStaticText(
				self, 
				-1, 
				label = text_list[i],
				size = (60, 30), 
				style = wx.ALIGN_CENTRE))
			self.label_list[i].SetBackgroundColour((238, 228, 218))
			self.play_board.Add(self.label_list[i], flag = wx.EXPAND|wx.RIGHT|wx.TOP, border = 10)
	...
	def refresh(self):
		...
		for i in range(0, self.SIZE * self.SIZE):
			self.label_list[i].SetLabel(text_list[i])
